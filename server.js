...
app.get('/api/dashboard', requireAuth, async (req, res) => {
  try {
    ...
    if (!skipStats && (taskType === 'delegation' || taskType === 'both')) {
      const [[u1]] = await db.query(`SELECT COUNT(*) AS cnt FROM delegation_tasks t WHERE status IN ('pending','revised') AND YEAR(due_date)=YEAR(CURDATE()) AND MONTH(due_date)=MONTH(CURDATE()) ${userFilter}`, params);
      upcoming += parseInt(u1.cnt)||0;
    }
    if (!skipStats && (taskType === 'checklist' || taskType === 'both')) {
      const [[u2]] = await db.query(`SELECT COUNT(*) AS cnt FROM checklist_tasks t WHERE status IN ('pending','revised') AND YEAR(due_date)=YEAR(CURDATE()) AND MONTH(due_date)=MONTH(CURDATE()) ${userFilter}`, params);
...
// Temporary test endpoint — sends to personal number 6367577176 (DELETE AFTER TEST)
